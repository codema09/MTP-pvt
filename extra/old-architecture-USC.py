...
BPF_HASH(active_connections, u64, struct conn_tuple_t);

// Map to store socket pointer to connection info
BPF_HASH(sock_to_conn, u64, struct conn_tuple_t);

// Map PID to most recent connection info (for multi-threaded servers)
BPF_HASH(pid_to_conn, u32, struct conn_tuple_t);
...
// Trace TCP accept to capture connection 4-tuple
TRACEPOINT_PROBE(syscalls, sys_exit_accept4) {
    ...
    bpf_trace_printk("Accept4 returned FD: %d for PID: %d\\n", fd, pid);
...
int probe_ssl_read_exit(struct pt_regs *ctx) {
    ...
    // Try to get connection info - check multiple sources
    u64 conn_key = pid_tgid;
    struct conn_tuple_t *conn = active_connections.lookup(&conn_key);
    if (conn != NULL) {
        ...
        // Fallback: check by PID (for multi-threaded servers)
        conn = pid_to_conn.lookup(&pid);
        if (conn != NULL) {
            evt->src_ip = conn->src_ip;
            evt->dst_ip = conn->dst_ip;
            evt->src_port = conn->src_port;
...
// Trace socket operations at kernel level to get connection info
int trace_tcp_v4_connect(struct pt_regs *ctx, struct sock *sk) {
    ...
    struct conn_tuple_t conn = {
        ...
        .dst_port = bpf_ntohs(sk->__sk_common.skc_dport),
    };
...
int trace_inet_csk_accept_exit(struct pt_regs *ctx) {
    ...
    // For accepted connections, the source is the remote client
    struct conn_tuple_t conn = {
        .src_ip = newsk->__sk_common.skc_daddr,  // Remote IP
        .dst_ip = newsk->__sk_common.skc_rcv_saddr,  // Local IP
        .src_port = bpf_ntohs(newsk->__sk_common.skc_dport),  // Remote port
        .dst_port = newsk->__sk_common.skc_num,  // Local port
    };
    
    // Store by pid_tgid (for this thread)
    active_connections.update(&pid_tgid, &conn);
    
    // IMPORTANT: Store by PID so all threads in this process can access it
    pid_to_conn.update(&pid, &conn);
    
    // Also store by socket pointer for backup
    u64 sock_ptr = (u64)newsk;
    sock_to_conn.update(&sock_ptr, &conn);
    
    bpf_trace_printk("Accept: PID=%d TID=%d\\n", pid, (u32)pid_tgid);
    
    return 0;
}

// Hook into socket receive to track which thread is handling which connection
TRACEPOINT_PROBE(syscalls, sys_enter_recvfrom) {
    u64 pid_tgid = bpf_get_current_pid_tgid();
    u32 pid = pid_tgid >> 32;
    
    // Look up connection for this pid_tgid
    struct conn_tuple_t *conn = active_connections.lookup(&pid_tgid);
    if (conn != NULL) {
        // Store the connection info for this PID so all threads can find it
        pid_to_conn.update(&pid, conn);
        // Also store by this specific tid for faster lookup
        active_connections.update(&pid_tgid, conn);
    } else {
        // Try to get from PID map and store locally
        conn = pid_to_conn.lookup(&pid);
        if (conn != NULL) {
            active_connections.update(&pid_tgid, conn);
        }
...
// Track socket read operations to propagate connection info to new threads
TRACEPOINT_PROBE(syscalls, sys_enter_read) {
    u64 pid_tgid = bpf_get_current_pid_tgid();
    u32 pid = pid_tgid >> 32;
    
    // Check if we already have connection info for this thread
    struct conn_tuple_t *existing = active_connections.lookup(&pid_tgid);
    if (existing == NULL) {
        // Try to get from PID-level map (set by parent thread)
        struct conn_tuple_t *conn = pid_to_conn.lookup(&pid);
        if (conn != NULL) {
            // Inherit connection info from parent/sibling thread
            active_connections.update(&pid_tgid, conn);
        }
    }
