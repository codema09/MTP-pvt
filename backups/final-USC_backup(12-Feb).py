...
// ═══════════════════════════════════════════════════════════════
// NEW: CPU BURST & CYCLE TRACKING (unified in sched_switch)
// ═══════════════════════════════════════════════════════════════

// We handle BOTH switch-out (prev) and switch-in (next) in a single
...
TRACEPOINT_PROBE(sched, sched_switch) {
    ...
    // We cannot get TGID from tracepoint args alone, so we look up
    // whether this TID already has a request tracked (meaning it
    // belongs to our target). Alternatively, we optimistically
    // record and let the lookup fail later if it's not ours.
    // For correctness we check /proc-style via a helper map:
    // tid_to_request_id will only have entries for TARGET_PID threads.
    u32 next_tid = args->next_pid;
    
    // Quick check: only record if this TID is tracked by us
    // (has an entry in tid_to_request_id or tid_to_thread_start)
    int is_tracked = 0;
...
class IntegratedSnifferFull:
    ...
    def run(self):
        ...
        self.bpf = BPF(text=BPF_PROGRAM_TEMPLATE % (self.pid, num_cpus))
