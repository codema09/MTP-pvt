...
class IntegratedSniffer:
    ...
    def display_user_history(self, username, history):
        ...
        header = f"| {'#':<3} | {'Request ID':<20} | {'Time':<8} | {'Thread':<7} | {'Source IP:Port':<21} | {'Mmap (B)':>12} | {'Mprot (B)':>12} | {'Overhead (ns)':>15} | {'Lifetime (ms)':>15} |"
        separator = '+' + '-' * 5 + '+' + '-' * 22 + '+' + '-' * 10 + '+' + '-' * 9 + '+' + '-' * 23 + '+' + '-' * 14 + '+' + '-' * 14 + '+' + '-' * 17 + '+' + '-' * 17 + '+'
        ...
        if history.request_count == 0:
            ...
            for i in range(history.request_count):
                ...
                try:
                    ...
                    mprotect_bytes = usage.mprotect_bytes
                except KeyError:
                    pass
                
                print(f"| {i+1:<3} | {req_id:<20} | {time_str:<8} | {req_entry.tid:<7} | {source_str:<21} | {mmap_bytes:>12,d} | {mprotect_bytes:>12,d} | {overhead_ns:>15,d} | {lifetime_ms:>15.2f} |")
    ...
    def display_resource_summary(self):
        ...
        try:
            ...
            header = f"| {'Request ID':<20} | {'Duration (ms)':>15} | {'Mmap (B)':>12} | {'Mprot (B)':>12} | {'Overhead (ns)':>15} | {'Lifetime (ms)':>15} | {'Memory (KB)':>12} | {'Status':<10} |"
            separator = '+' + '-'*22 + '+' + '-'*17 + '+' + '-'*14 + '+' + '-'*14 + '+' + '-'*17 + '+' + '-'*17 + '+' + '-'*14 + '+' + '-'*12 + '+'
            ...
            for req_id_key, usage in sorted(resource_map.items(), key=lambda item: item[1].start_time_ns):
                ...
                print(f"| {req_id:<20} | {duration_ms:>15.2f} | {mmap_bytes:>12,d} | {mprotect_bytes:>12,d} | {overhead_ns:>15,d} | {lifetime_ms:>15.2f} | {mem_kb:>12,d} | {status:<10} |")
