...
// Accumulates CPU bursts for threads not yet assigned a request_id
BPF_HASH(tid_cpu_pre_assign, u32, struct pre_assign_cpu_t);
// TID → TGID registry so PART 2 can identify target-process threads without needing TGID
BPF_HASH(tid_to_tgid, u32, u32);
// Per-CPU scratch for debug burst events (avoids large BPF stack allocations)
BPF_PERCPU_ARRAY(cpu_burst_scratch, struct cpu_burst_event_t, 1);
BPF_PERF_OUTPUT(cpu_burst_events);
