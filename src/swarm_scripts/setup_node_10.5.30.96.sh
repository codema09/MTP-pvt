...
# Node 10.5.30.96 Setup Script

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
cd "$DIR"

echo "=========================================="
echo "    Deploying Services for 10.5.30.96   "
echo "=========================================="

if [ "false" == "true" ]; then
    echo "[1/4] Initializing Docker Swarm as Manager on 10.5.30.96..."
    docker swarm init --advertise-addr 10.5.30.96 || echo "Already in a swarm"
    ...
    if [ -z "$1" ]; then
        ...
        echo "Usage: ./setup_node_10.5.30.96.sh <SWARM_JOIN_TOKEN>"
        exit 1
    fi
    TOKEN=$1
...
docker compose -f docker-compose-10.5.30.96.yml up -d
...
PIDS=""
for container in $(docker compose -f docker-compose-10.5.30.96.yml ps -q); do
...
cd ..
sudo nohup python3 new-architecture-USC.py -p $PIDS > "sniffer_10.5.30.96.log" 2>&1 &

echo "==========================================================="
echo "✅ Setup Complete!"
echo "Docker services are UP."
echo "Sniffer is detached and logging to: src/sniffer_10.5.30.96.log"
echo "Check \`tail -f src/sniffer_10.5.30.96.log\` for live memory profiling."
echo "==========================================================="
